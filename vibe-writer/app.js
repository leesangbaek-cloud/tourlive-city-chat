// Vibe Writer App Logic (v1.9 Stable - Reverted)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Crawling Functionality (Verified Stable)
    fetchUrlBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.contents) {
                ...
                sourceUrl.style.borderColor = '#10b981';

                alert('자료수집 완료! 이제 이 내용을 바탕으로 대본을 생성할 수 있습니다.');
            }
        } catch (error) {
    ...
    // Generate Script using Premium Dynamic Templates (Reverted to Stable Logic)
    generateBtn.addEventListener('click', () => {
        ...
        let topic = "이 작품";
        const topicMatch = sourceText.match(/\[Source:\s*(.*?)[\]\n]/);
        if (topicMatch) topic = `"${topicMatch[1].split('|')[0].trim()}"`;
...
오늘 여러분의 마음속에는 어떤 영감이 소용돌이치고 있나요? [Pause] 이 아름다운 소동을 뒤로하고, 우리는 이제 이 흐름이 이어지는 다음 장소로 발걸음을 옮겨보겠습니다.`,
                ...
                orsay: `
# 도입
${topic}... 이 이름만 들어도 가슴 한구석이 아릿해지는 느낌입니다. [Pause] 수집된 ${topic}의 자료들 사이로 고개를 내밀고 있는 감정의 조각들을 하나씩 모아봤습니다. 마치 누군가의 간절한 기도처럼 반짝이는 이야기들입니다.
